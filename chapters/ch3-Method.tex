...
\label{sec:Method}
This study consists of agentic-simulation analysis to explore the relationship between occupancy-related parameters to the effect of control performance based on different types of \ac{gcm} using data collected in a real-world office building.
...
For the analysis, three primary types of field data were collected: real-time occupancy logs, subjective thermal comfort surveys and environmental measurement logs.

\subsection{Individual-level location tracking log}
Individual-level location tracking log are reconstructed from anonymized, timestamped location data provided by the \ac{bms}.
...
\paragraph{Thermal Comfort Survey}
...
To ensure sufficient variability of answers, indoor \ac{ti} setpoints were cycled between \qtyrange{22}{27}{\celsius} in \qty{1}{\celsius} steps, with randomized order across days.  
...
The survey responses were later classified into sessions defined by room, date, and merged survey timeslot.

\paragraph{Indoor Environmental Measurements}
Indoor environmental data were monitored in parallel with \ac{tcv} survey data.
