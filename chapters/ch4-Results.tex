...
Figure~\ref{fig:ComfAgentic}\subref{fig:ComfAchi} shows the comfort probability ratio using \ac{sgcm}, \ac{dgcm} and \ac{rtgcm} for different subgroup sizes across four office rooms.
...
The \ac{rtgcm} shows the highest comfort achievement ratio for every room and subgroup size,followed by \ac{dgcm} and then \ac{dgcm}.
The performance of \ac{rtgcm} and \ac{dgcm} shows decreasing trend with similar ratio as subgroup size increases, while the performance of \ac{sgcm} shows more modest decrease as subgroup size increases.
The mean comfort achievement ratio across rooms ranged approximately  \qtyrange{80}{98}{\percent} for the \ac{rtgcm}, \qtyrange{75}{95}{\percent} for the \ac{dgcm}, and \qtyrange{65}{73}{\percent} for the \ac{sgcm}.
...
Although both \ac{dgcm} and \ac{rtgcm} keeps improvement of performance from baselines, the gap between the \ac{dgcm} and \ac{rtgcm} increases as room size expands.
...
\subsection{Effect of \ac{rtgcm} to Control Adjustment}
\label{subsec:controleffect}
In previous subsection, the comfort performance of \ac{rtgcm} is shown to keep higher by \qtyrange{15}{30}{percent} compared to \ac{sgcm} across room member sizes.
This subsection investigates the effect of \ac{rtgcm} to control adjustment.
Since {sgcm} and {dgcm} assumes static occupant composition, the comtrol adjustment is only required in \ac{rtgcm}-based control.
...
\subsection{Effect of Occupancy dynamics to Control action frequency}
...
It is observed that as \ac{utr} increases, the probability of required temperature adjustment also increases, the highest range such as 0.7--0.8, the probability of temperature adjustment reaches to \qty{90}{\percent}.
