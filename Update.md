...
## 8. Journal稿を直すための作業ガイド（平易な日本語版、2026-08-26）

### 8.1 この章の使い方

この章は、第7章の調査結果を、実際の修正作業に使える形へ並べ直したものである。
第7章は判断根拠を詳しく確認したいときに参照し、通常の作業では本章を上から順に進める。

各項目には、次の4点を示す。

- **対象**: 修正するファイル
- **場所**: ファイル内で検索する文章または見出し
- **作業**: 削除、追加、置換のどれを行うか
- **完了条件**: 修正が正しいか確認する方法
...
|---|---|
| 今すぐ修正 | 解析結果を待たずに直せる |
| 確認後に修正 | 元データや生成コードを確認してから直す |
| 保留 | 現時点では本文へ入れない |

**最も重要な注意:** ASIM稿の数値をそのままJournal稿へコピーしない。
comfortの評価方法、設定温度の決め方、15分と30分の扱いが確定した後、同じ解析結果表から数値を入れ直す。

### 8.2 最初に行う8つの作業

| 順番 | 作業 | 主な対象 | 完了条件 |
|---:|---|---|---|
| 1 | comfortの評価方法を決める | 解析コード、Method | binary判定、予測確率、normalized scoreのいずれかを決め、名称を統一している |
| 2 | 3つのGCMで設定温度の決め方をそろえる | 解析コード、Method | SGCM、DGCM、RT-GCMが同じ計算方法を使う |
| 3 | 15分と30分の役割を決める | Method、Results、図 | 本文、式、図、captionの時間幅が一致する |
| 4 | ASIM稿とJournal稿の部屋名を対応させる | Method、Results、図 | 4室のIDが全ファイルで一致する |
| 5 | 同じ条件で結果を再計算する | 解析コード、source table | 全図と全数値を同じ解析条件から作れる |
| 6 | Methodを修正する | chapters/ch3-Method.tex | 実際の解析方法と本文が一致する |
| 7 | Resultsを修正する | chapters/ch4-Results.tex | 数値、単位、図、captionがsource tableと一致する |
| 8 | Abstract、Discussion、Conclusionを直す | Draft_2026-Jan.tex、ch5、ch6 | Resultsと同じ数値だけが使われている |

### 8.3 最初に決める4項目

#### 8.3.1 comfortを何で評価するか

- **状態:** 確認後に修正
- **対象:** 解析コード、chapters/ch3-Method.tex、chapters/ch4-Results.tex
- **現在の問題:** Methodは「No changeの予測確率の平均」と説明しているが、現在のJournal図は「設定温度が各PCMの快適範囲に入るか」という0/1判定を使っている可能性がある。
- **決めること:** comfortを予測確率で評価するか、0/1の範囲内判定で評価するか。
- **推奨:** 予測確率を使い、Methodの説明と合わせる。
- **追加確認:** rawまたはcalibrated predicted probabilityを使うのか、各PCMのpeakで正規化したscoreを使うのかを決める。正規化した値を使う場合は comfort probability と呼ばず、normalized comfort score など別の名前にする。
- **baselineの扱い:** 固定24 degCも、3つのGCMと同じ在室者集合、同じcomfort指標、同じ平均方法で評価する。
- **完了条件:** Methodの式、解析コード、図の縦軸、Resultsの用語がすべて同じ指標を示す。

予測確率を使う場合、SGCM、DGCM、RT-GCMのいずれも、次の同じ手順で設定温度を決める。

1. その時点で各GCMが代表する人の予測確率曲線を平均する。
2. 同じ候補温度の中から、平均予測確率が最大になる温度を選ぶ。
3. 実際に在室している人のPCMへ戻してcomfortを評価する。

#### 8.3.2 3つのGCMで設定温度の決め方をそろえる

- **状態:** 確認後に修正
- **現在の問題:** 確認できたworkerでは、SGCMはPeakSETの平均、DGCMとRT-GCMは平均曲線の最大点を使っている。このままでは、時間解像度の違いだけでなく、計算方法の違いも結果へ入る。
- **作業:** 3つのGCMに同じ「平均曲線の最大点」を使う。
- **完了条件:** 3つのGCMの違いが、誰を平均するかだけになっている。

比較する人の集合は次のとおりとする。

- SGCM: その部屋から抽出したregular members
- DGCM: その日に来たattendees
- RT-GCM: その時点に在室しているoccupants

#### 8.3.3 15分と30分のどちらを主結果にするか

- **状態:** 確認後に修正
- **対象:** chapters/ch3-Method.tex、chapters/ch4-Results.tex、fig:CtrlWindowRatio
- **現在の問題:** 図のファイル名は15分、Results本文は30分、captionには15分と30分由来の言葉が混在している。
- **先に確認すること:**
  - fig:CtrlWindowRatioは15分ごとの変化を集計した図か。
  - MethodのAWR式は30分窓の集計か。
  - センサー記録、stay log、HVAC運用のどれを理由に時間幅を選ぶか。

**15分を主結果にする場合**

- 30分AWRの式と説明を主解析から外すか、補助解析へ移す。
- 連続する15分時点のうち、有効かつ在室中のtransitionを分母とし、丸めた設定温度が0.5 degC以上変わったtransitionを分子とする。ただし、最終定義は生成コードと一致させる。
- Resultsとcaptionでは actionable-update rate と呼ぶ。
- 37%から89-93%という数値は、15分のsource tableと一致した場合だけ使う。

**30分を主結果にする場合**

- Methodの30分AWR式を残す。
- ASIMの15分図と37%から89-93%の数値は使わない。
- 30分AWRの元データから図とResultsを作り直す。

- **完了条件:** 全文検索で 15-minute、30-minute、AWR、actionable-update rate を確認し、主指標と補助指標が区別できる。

#### 8.3.4 部屋名の対応を決める

- **状態:** 確認後に修正
- **対象:** chapters/ch3-Method.tex、chapters/ch4-Results.tex、図中のroom label
- **現在の問題:** Journal稿とASIM稿で部屋名が異なり、MondayのUTR peakを示す部屋の対応にも不整合がある。
- **作業:** 元のroom IDを使って、Journal名とASIM名の対応表を作る。推測では置換しない。
- **完了条件:** Methodの表、Results本文、図、captionの4室名が同じ対応表に従っている。

### 8.4 ファイル別の具体的な修正指示

#### 8.4.1 chapters/ch3-Method.tex

##### A. location trackingの説明を具体的にする

- **状態:** 元資料確認後に修正
- **場所:** Individual-level location data were obtained from anonymized, timestamped records... を検索する。
- **作業:** 現在の最初の2文だけを置き換える。ID、room ID、start time、end timeを説明する後続文は残す。

> Anonymized individual-level location records were obtained from a face-recognition system installed at room entrances and in corridors. The timestamped zone-level records were reconstructed into continuous room-stay intervals with anonymized IDs and inferred start and end times.

- **確認事項:** face-recognition systemがBMSの一部か別システムかを確認する。start/end timeを後処理で推定した場合だけ inferred を使う。直接記録された時刻なら表現を直す。
- **残す文章:** IRB承認、参加者の同意、管理者が事前に匿名化した説明は削除しない。
- **追加確認:** 検出漏れ、重複記録、滞在区間の再構築方法がAppendixにある場合は相互参照する。精度を評価していない場合はLimitationsに記載する。
- **完了条件:** 取得方法、設置場所、匿名化、stay logへの変換が分かる。

##### B. data collection 7室とsimulation 4室を分ける

- **状態:** 部屋名の対応確認後に修正
- **場所:** Data collection was limited to the following rooms: ... の直後。
- **作業:** 「データを集めたのは7室、simulation結果を示すのは4室」と説明する1文と4室表を追加する。
- **使用候補:** ref/asim2026_import_candidates/figures/TargetRooms.tex
- **注意:** room mappingが確定するまで表を本文へ入れない。
- **完了条件:** 読者が7室と4室を混同せず、Resultsのroom labelと表が一致する。

##### C. 0.1 degCと0.5 degCの違いを説明する

- **状態:** 元コード確認後に修正
- **場所:** where $\mathcal{T}_{\mathrm{set}}$ is the temperature grid of candidate setpoints. の直後。
- **作業:** 次を追加する。

> The candidate setpoint grid had a resolution of \qty{0.1}{\celsius}. This grid was used only to search for the comfort-optimal setpoint in the simulation. In the separate actionable-update analysis, setpoints were rounded to \qty{0.5}{\celsius} before determining whether a change would require an operational update.

- **完了条件:** 0.1 degCを実建物の制御分解能と誤解しない説明になっている。

##### D. 15分または30分の定義をそろえる

- **状態:** 8.3.3を決めた後に修正
- **場所:** At each 30-minute control timestep、UTRの式、AWRの式と説明。
- **作業:** 選んだ主指標に合わせ、時間幅、分母、空室、欠測、日境界を明記する。
- **追加確認:**
  - 前後とも空室でUTRの和集合が空になる場合を除外するか、0とするか。
  - 0.5 degCへ丸める途中値をどちらへ丸めるか。
  - 欠測で時間が飛んだ区間を除外するか。
- **完了条件:** 式だけを読んでも同じ値を再計算できる。

##### E. Monte Carlo条件を明記する

- **状態:** 実run確認後に修正
- **場所:** For each room and subgroup size, 1000 Monte Carlo trials... の前後。
- **作業:** location-log agentとPCMの母集団、復元抽出の有無、random seed、trial数、temperature gridを追記する。
- **作業:** 同じtrialで3つのGCMが同じPCM割当を共有することも明記する。
- **注意:** ASIM図名の raw2000 がtrial数を示すなら、Methodの1,000 trialsと一致しない。確認するまで数値を固定しない。

#### 8.4.2 chapters/ch4-Results.tex

##### A. 同じ画像の二重掲載を直す

- **状態:** 今すぐ確認して修正
- **場所:** fig:MeanOccuConf と fig:ImpGCDMvsRTGCM。
- **現在の問題:** 2つのfigure環境が同じ画像 analysis21_a6_style_rt_minus_dgcm... を参照している。
- **作業:** 本文が参照していない fig:MeanOccuConf 側のfigure環境を削除し、本文が参照している fig:ImpGCDMvsRTGCM を残す。
- **完了条件:** 同じ画像が連続して2回表示されず、未使用labelや壊れた参照がない。

##### B. captionの重複語を直す

- **状態:** 今すぐ修正
- **場所:** fig:ComfAchiのcaption。
- **現在:** Mean comfort probability probability under different comfort-representation controls
- **修正後:** Mean comfort probability under different comfort-representation controls

##### C. comfortの数値を入れ直す

- **状態:** 8.3.1と8.3.2の再解析後に修正
- **場所1:** with RT-GCM improvement remaining approximately 15-20 percent...
- **場所2:** The previous subsection showed that the RT-GCM maintained a 15-30 percent higher...
- **作業:** 上の数値をいったん検証対象とし、新しいsource tableから入れ直す。
- **修正範囲:** comfort subsection全体を対象とする。固定24 degCの65-70%、RT-GCMの80-98%、DGCMの75-95%、SGCMの65-73%、K=5-7でSGCMがzeroへ近づく主張、全room/KでRT-GCMが最高という主張、policy順位、傾向、全captionを新しいsource tableから再確認する。
- **Discussion:** chapters/ch5-Discussion.texにあるSGCMがK=5-7でzeroへ近づく主張と、旧comfort図に基づく順位・傾向も同時に再確認する。
- **書き方:** 基準との差を引き算した値は percent higher や improvement ratio ではなく percentage points と書く。

修正文の型は次のとおり。角括弧は確認後に実数へ置き換え、角括弧のまま本文へ入れない。

> At the largest tested subgroup sizes, the RT-GCM exceeded the fixed \qty{24}{\celsius} baseline by [xx--xx] percentage points and exceeded the DGCM by [xx--xx] percentage points.

- **使ってはいけない数値:** ASIM本文のK=2における96-97%。ASIMの実コンパイル図では約92-93%に見え、本文と図が一致していない。
- **完了条件:** Abstract、Results、Discussion、Conclusionの数値が同じsource tableと一致する。

##### D. subgroup sizeをroom sizeと呼ばない

- **状態:** 今すぐ修正
- **場所:** smaller rooms、room size、room member sizes など、Kの説明に使っている箇所。
- **置換:** smaller rooms は smaller sampled subgroup sizes へ変更する。
- **置換:** larger rooms は larger subgroup sizes へ変更する。
- **置換:** room member sizes は subgroup sizes へ変更する。
- **作業:** Kは sampled regular-member subgroup size と書く。実際の在室人数は $N_{r,t}$ と書く。
- **完了条件:** K、room capacity、実在室人数の3つが区別されている。

##### E. setpoint adjustmentの値を直す

- **状態:** 元のsource table確認後に修正
- **場所:** However, the mean value remains approximately around 1.0 degC... の文章。
- **作業:** changed stepsだけを対象にした図であることを明記し、次の形へ置き換える。

> Among timesteps at which the simulated setpoint changed, the plotted central value decreased from about \qty{1.3}{\celsius} near a daily mean occupancy of one to roughly \qty{1.0}{\celsius} near four and \qty{0.8}{\celsius} at the upper end of the observed range.

- **captionへ追加:** 25-75 percentileと10-90 percentileの帯であること。ただし、何の観測値を集計した帯かをコード確認後に書く。
- **注意:** 0.5 degCより大きいことだけで、実建物で実行可能とは断定しない。
- **追加削除対象:** This can be interpreted that ... practical in actual HVAC operation. の文は削除する。
- **理由:** HVACの応答時間、deadband、minimum hold time、actuator constraintsを評価していないため、現在のsimulationだけでは実行可能性を判断できない。

##### F. UTRと設定変更率の文章を直す

- **状態:** 15分を主結果に決め、source tableを確認した場合のみ修正
- **場所:** when UTR reaches approximately 0.7--0.8... の文章。
- **作業:** 15分図のbin別値が確認できたら、次の形へ置き換える。

> The actionable-update rate generally increased from approximately 37\% at a 15-minute UTR near 0.05 to 63--69\% at 0.4, 77--78\% at 0.65, and 89--93\% at 0.9--1.0.

- **注意:** 全binで単調増加を確認できた場合だけ monotonically を使う。それ以外は generally increased とする。
- **削除または弱める表現:** occupant replacement was the dominant driver。
- **理由:** UTRとの関連だけでは、実在室人数の影響と入れ替わりの影響を分けられない。
- **用語:** user turnover rate は occupant turnover rate へ統一する。

##### G. occupancy-density図を使う場合

- **状態:** 元集計とroom mappingの確認後に修正
- **挿入位置:** fig:ComfAgenticの後、comfort結果の説明文の前。
- **使用候補:** ref/asim2026_import_candidates/figures/occupancy_density_timeofday_k12_paper.png
- **Resultsへ書けること:** その時点の在室人数がregular-member poolより小さいこと。
- **Resultsへ書けないこと:** partial attendanceがRT-GCMとDGCMの差を生んだという因果関係。
- **完了条件:** 図番号、room label、intervalの定義、本文参照が一致する。

#### 8.4.3 Draft_2026-Jan.texのAbstract

- **状態:** Resultsの再解析後に修正
- **場所:** Draft_2026-Jan.tex内のbegin{abstract}からend{abstract}。現在はchapters/abstract.texがinputされていない。
- **今すぐ削除:** Further analysis shows that ... mean occupancy approaches four to five occupants. の文。
- **理由:** 対応する図が現在の本文になく、ASIM稿でも撤回されている。
- **作業:** 最終的にはchapters/abstract.texへ一本化するか、直書きAbstractと同期する。
- **最低限入れる内容:**
  - 何を評価したか: predicted person-time comfortとsetpoint-update burden
  - データ: 39 participants、four weeks、four simulation rooms
  - 2,608 responses: 収集総数か、20分除外後の保持数かを確認してから書く
  - 検証済みの主結果
  - predicted comfortを使ったsimulationであり、field-delivered comfortやenergy savingを示していないこと
- **完了条件:** Abstractの数値がResultsと同じsource tableに一致する。

#### 8.4.4 chapters/ch5-Discussion.tex

- **状態:** Results修正後に修正
- **削除または検証後に置換:** UTR has a strong effect on the probability of required temperature adjustment. の文。UTRとの関連だけを述べ、因果的な strong effect は使わない。
- **削除または検証後に置換:** RT-GCM、DGCM、SGCMをoccupancy条件別に推奨する段落。具体的には It is most valuable in sparse... と In contrast, a DGCM may provide... の2文。現在の解析だけでは導入条件を確定できない。
- **検索する表現:** room size、smaller rooms、dominant driver、strong effect。
- **作業:** Kをroom sizeと呼ばない。UTRとの関連を因果関係として書かない。
- **作業:** dynamic occupancy、tracking、implementation barrierの先行研究を、対応する結果の直後で再び引用する。
- **文章例:**

> Using observed occupancy logs and field-derived PCMs under a common group-to-setpoint rule, predicted comfort performance varied across subgroup-size and occupancy conditions, while actionable-update frequency was associated with occupant-composition turnover.

#### 8.4.5 chapters/ch6-Conclusion.tex

- **状態:** Results修正後に修正
- **現在の問題:** 定量結果がほとんどなく、どの程度の差があったか分からない。
- **削除または検証後に置換:** The advantage of the RT-GCM was strongest under sparse or dynamically changing occupancy conditions. の文。現行解析では sparse 条件で最も強いと確定できない。
- **削除または検証後に置換:** high-resolution occupancy tracking should be deployed selectively... の文。導入判断にはHVAC応答、privacy、sensing reliability、costの評価が必要である。
- **追加する順番:**
  1. 3つのGCMと固定24 degC baselineの比較
  2. 最大KでのRT-GCMとbaselineの差
  3. 最大KでのRT-GCMとDGCMの差
  4. changed-stepの代表値
  5. 15分または30分で統一したaction rate
  6. 未評価事項: energy、HVAC dynamics、privacy、cost、field validation
- **注意:** [xx]のまま本文へ入れない。すべて同じsource tableで確認してから追記する。

### 8.5 CSV 17件の対応先

| No. | どこを直すか | やること | 状態 |
|---:|---|---|---|
| 1 | Abstract、Introduction | OCC全体ではなく、room-level setpoint selectionを扱う研究だと限定する | 今すぐ修正 |
| 2 | title、keywords | titleと同じ語をkeywordで重ねるか投稿規定を確認する | 投稿先決定後 |
| 3 | affiliation | postcodeが必要か投稿規定を確認する | 投稿先決定後 |
| 4 | Abstract | predicted comfortとcontrol-update burdenを評価したと書く | Results確定後 |
| 5 | Method | face-recognition、設置場所、stay reconstruction、匿名化を書く | 元資料確認後 |
| 6 | Method、title page | 建物名と会社名を残すか匿名化方針を確認する | 投稿方針決定後 |
| 7 | Method | simulation対象4室の表を追加し、data collection 7室と分ける | room mapping後 |
| 8 | Results、Discussion | density図を示すか、partial-attendanceの説明を弱める | 元集計確認後 |
| 9 | Results | 根拠となるoccupancy-density figure番号を書く | 図採用後 |
| 10 | Method、Limitations | 15分または30分を選んだ理由を書く。可能なら感度分析を行う | 解析方法決定後 |
| 11 | Results caption | 25-75%と10-90%の帯、集計対象を説明する | 元コード確認後 |
| 12 | Method、図 | shared/assigned、room IDを表と図で一致させる | room mapping後 |
| 13 | Results、Discussion | 実在室人数で層別する。未実施ならdominant driverと書かない | 追加分析推奨 |
| 14 | Results、Conclusion | 最大Kのbaseline差を同一ruleの再解析で確認して書く | 再解析後 |
| 15 | Discussion | dynamic occupancy、tracking、implementationの文献を結果へ接続する | Results確定後 |
| 16 | Method、Discussion | 比較要因はaggregation resolution、適用条件はdynamic occupancyと書き分ける | 今すぐ修正 |
| 17 | Conclusion | comfort、DGCMとの差、setpoint step、action rateの代表値を書く | Results確定後 |

### 8.6 ASIM稿からコピーしたファイルの扱い

保存先は ref/asim2026_import_candidates/ である。ASIM Conference側のファイルは変更していない。

| ファイル | 用途 | 使用条件 | 現在の扱い |
|---|---|---|---|
| occupancy-density図 | partial attendanceの説明 | room名、元集計、interval定義を確認 | 保留 |
| TargetRooms.tex | simulation 4室表 | Journal名とのroom mappingを確認 | 保留 |
| figures/fig1_analysis23nr_with_fixed24_raw2000_64_100_all_solid_small_markers_paper.png | comfort結果 | 生成コード、3 policyのrule、trial数を確認 | 使用しない |
| figures/fig3a_setpoint_adjustment_by_occupancy_compact_paper.png | setpoint adjustment | source table、changed-step分母、central value、percentileの単位を確認 | 保留 |
| figures/fig3b_setpoint_adjustment_by_group_size_and_occupancy_density_compact_paper.png | setpoint adjustment heatmap | bin定義とblank-cell thresholdを確認 | 保留 |
| figures/fig4a_action_rate_by_turnover_compact_paper.png | 15分UTR/action rate | transition分母、空室、日境界、rounding、bin、source tableを確認 | 保留 |
| figures/fig4b_weekly_30min_utr_selected_compact_paper.png | 30分weekday UTR | 15分captionとの不一致とroom mappingを解消 | 保留 |
| figures/SimulationFlow_small.png | Method workflow | 本文で確定した共通policy ruleと一致させる | 保留 |
| bib/wang_enhancing_2026.bib | Discussion | 本研究と対象規模・指標が違うと説明する | 引用時に使用可 |

### 8.7 現時点では本文へ入れない内容

- ASIM本文のK=2 comfort 96-97%。実コンパイル図と一致しない。
- ASIM図名 raw2000 に由来するtrial数。Methodの1,000 trialsとの関係が未確認。
- occupant replacement was the dominant driver という断定。
- mean occupancyが4-5人になるとRT-GCMの効果がなくなるという主張。
- 15分を選んだ理由。元資料にない理由を推測で書かない。
- energy savings、実建物でのcomfort改善、cost effectiveness。
- room mappingを確認しないまま行う部屋名の置換。

### 8.8 解析結果を確定するときに保存する一覧

再解析時は、次を1つのanalysis manifestまたはREADMEへ残す。

- 使用したデータのsnapshotと日付
- 対象4室とroom mapping
- Kの範囲と各室の最大K
- trial数、random seed、コードのversionまたはhash
- location-log agentとPCMの選択条件
- 復元抽出か非復元抽出か
- 同じtrialで3 policyが同じPCM割当を使ったか
- SGCM、DGCM、RT-GCMの人の集合を作る規則
- 温度候補の範囲、刻み、最大値が複数あるときの選び方
- comfortの平均方法。全person-timeをまとめて平均するのか、room-day内で平均してから各日を等重みにするのかを決める。
- room、day、trial、Kをどの順番で集計するか。欠測dayとroom間の重みも決める。
- 欠測日、空室、日境界の扱い
- UTRとaction rateの分母、bin、各binの件数
- uncertainty bandまたはCIの観測単位、再標本化単位、percentileかbootstrap CIか、各binの件数
- comfort図、baseline差、RT-GCMとDGCMの差を同じsource tableから作ったこと

Limitationsには、同じoccupancy logを再利用したMonte Carlo trialのばらつきには、PCM推定誤差や別期間・別建物へ一般化するときの不確実性が含まれないことを追記する。

### 8.9 独立レビュー結果

独立レビュアーは、第7章の科学的な方向性は妥当と判断した。
特に、次の方針は維持すべきである。

- 3つのGCMで設定温度の決め方をそろえる。
- binary comfortと予測確率を混ぜない。
- 15分と30分を本文、式、図で統一する。
- occupancy densityだけでRT-GCMの効果の原因を断定しない。
- Monte Carlo trialのばらつきを母集団のconfidence intervalと呼ばない。
- energy savingやfield-delivered comfortを結果として主張しない。

レビュアーから追加された主な修正は次のとおりである。

- chapters/ch4-Results.texで同じ画像の二重掲載を確認したため、未参照のfig:MeanOccuConf側を削除し、fig:ImpGCDMvsRTGCMを残す。
- Mean comfort probability probability の重複語を直す。
- Kをroom sizeやsmaller roomsと言い換えない。
- 2,608 responsesが収集総数か除外後の件数かを確認する。
- UTRで前後とも空室の場合と、0.5 degC丸めの中間値の扱いを決める。

### 8.10 最終確認チェックリスト

- [ ] 15-minute、30-minute、AWRを全文検索し、主指標と補助指標が区別されている。
- [ ] improvement ratio、percent higherを検索し、差を表す箇所はpercentage pointsへ直している。
- [ ] user turnover rateをoccupant turnover rateへ統一している。
- [ ] K、room capacity、実在室人数 $N_{r,t}$を混同していない。
- [ ] 旧room名と新room IDが混在していない。
- [ ] Abstract、Results、Discussion、Conclusionの数値が同じsource tableと一致している。
- [ ] Method、図の軸、caption、Resultsで指標名と時間幅が一致している。
- [ ] 同じ画像の二重掲載がない。
- [ ] 図番号、相互参照、未使用labelに問題がない。
- [ ] energy saving、field comfort、costを評価済みのように書いていない。
- [ ] LaTeXコンパイル後、表幅、図、単位、参照番号を目視確認している。
